...
def find_as_events(gtf, prefix, output_dir, logger):
    ...
    oFile_3 = open(os.path.join(output_dir, "A3SS.csv"), "w")  ## alt-3 SS output file
    oFile_5 = open(os.path.join(output_dir, "A5SS.csv"), "w")  ## alt-5 SS output file
    oFile_ce = open(os.path.join(output_dir, "SE.csv"), "w")  ## skipped exon (cassette exon) output file
    oFile_mxe = open(os.path.join(output_dir, "MXE.csv"), "w")  ## Mutually exclusive exon output file
    oFile_afe = open(os.path.join(output_dir, "AFE.csv"), "w")  ## alt first exon output file
    oFile_ale = open(os.path.join(output_dir, "ALE.csv"), "w")  ## alt last exon output file
    oFile_ri = open(os.path.join(output_dir, "RI.csv"), "w")  ## retained intron output file
    #
    # neFile_3 = open(os.path.join(output_dir, "novelEvents.A3SS.csv"), "w")  ## alt-3 SS novel events output file
    # neFile_5 = open(os.path.join(output_dir, "novelEvents.A5SS.csv"), "w")  ## alt-5 SS novel events output file
    # neFile_ce = open(os.path.join(output_dir, "novelEvents.SE.csv"), "w")  ## skipped exon (cassette exon) novel events output file
    # neFile_mxe = open(os.path.join(output_dir, "novelEvents.MXE.csv"), "w")  ## mxe novel events output file
    # neFile_ri = open(os.path.join(output_dir, "novelEvents.RI.csv"), "w")  ## ri events output file
    # neFile_afe = open(os.path.join(output_dir, "novelEvents.AFE.csv"), "w")  ## afe events output file
    # neFile_ale = open(os.path.join(output_dir, "novelEvents.ALE.csv"), "w")  ## ale events output file
    #
    # samFiles =   ## coulbe be multiple sam files
    # #
    ...
    for line in iFile:  ## for each line
        ...
        group = range(int(int(sC) / chunk), int(int(eC) / chunk) + 1)  ## groups this line could belong to
